...
const PrayerWall = () => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (!newPrayer.text.trim()) {
            return toast.error("Por favor escribe tu petición");
        }

        setIsSubmitting(true);
        setTimeout(() => {
            const memberName = currentMember ? `${currentMember.nombre} ${currentMember.apellido.charAt(0)}.` : "Anónimo";
            const prayer: Prayer = {
                id: Date.now(),
                author: newPrayer.isAnonymous ? "Anónimo" : memberName,
                ...
                <div className="container mx-auto px-4 text-center">
                    <div className="max-w-md mx-auto">
                        ...
                        <p className="text-slate-600 dark:text-slate-400">
                            Tu acceso al Muro de Oración ha sido restringido. Si crees que esto es un error, contacta a la administración de la iglesia.
                        </p>
                    </div>
                </div>
            </section>
