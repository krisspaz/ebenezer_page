...
const FALLBACK_DATA: Magazine[] = [
    { title: "ENERO 2026", link: "https://ebenezer.org.gt/wp-content/uploads/2026/01/190-Enero2026.pdf", image: "https://ebenezer.org.gt/wp-content/uploads/2026/01/IMG_3582.jpeg" },
...
export async function fetchRhemaMagazines(): Promise<Magazine[]> {
    try {
        // Try real-time API first for immediate updates
        const apiResponse = await fetch('/api/rhema');
        if (apiResponse.ok) {
            const apiData = await apiResponse.json();
            const result = MagazineListSchema.safeParse(apiData);
            if (result.success && result.data.length > 0) {
                console.log('✅ Magazines loaded via Real-Time API');
                return result.data;
            }
        }

        // Fallback to static JSON if API fails or returns empty
        console.log('🔄 API failed or empty, falling back to static JSON...');
